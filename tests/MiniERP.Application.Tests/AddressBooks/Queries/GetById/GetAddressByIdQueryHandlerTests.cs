using FluentAssertions;

using FluentResults;

using MiniERP.AddressBook.Domain.Entities;
using MiniERP.Application.Abstractions;
using MiniERP.Application.Addresses.Dtos;
using MiniERP.Application.Addresses.Queries.GetById;
using MiniERP.Users.Domain.Entities;

using Moq;

namespace MiniERP.Application.Tests.AddressBooks.Queries.GetById;

public class GetAddressByIdQueryHandlerTests
{
    private readonly Mock<IRepository<Address>> _mockAddressRepository;
    private readonly Mock<IRepository<User>> _mockUserRepository;
    private readonly Mock<IMapper<Address, AddressDto>> _mockAddressMapper;
    private readonly Mock<IMapper<User, AddressUserDto>> _mockUserMapper;
    private readonly GetAddressByIdQueryHandler _handler;

    public GetAddressByIdQueryHandlerTests()
    {
        _mockAddressRepository = new Mock<IRepository<Address>>();
        _mockUserRepository = new Mock<IRepository<User>>();
        _mockAddressMapper = new Mock<IMapper<Address, AddressDto>>();
        _mockUserMapper = new Mock<IMapper<User, AddressUserDto>>();
        _handler = new GetAddressByIdQueryHandler(
            _mockAddressRepository.Object,
            _mockUserRepository.Object,
            _mockAddressMapper.Object,
            _mockUserMapper.Object);
    }

    [Fact]
    public async Task Handle_ShouldReturnAddress_WhenAddressExists()
    {
        // Arrange
        var addressId = 1;
        var userId = 1;
        var address = new Address { Id = addressId, Street = "123 Main St", City = "Anytown", State = "Anystate", PostalCode = "12345", Country = "USA", IsPrimary = true, UserId = userId };
        var user = new User { Id = userId, FirstName = "John", LastName = "Doe", Email = "john.doe@example.com" };
        var addressDto = new AddressDto { Id = addressId, Street = "123 Main St", City = "Anytown", State = "Anystate", PostalCode = "12345", Country = "USA", IsPrimary = true, User = new AddressUserDto { Id = userId, FirstName = "John", LastName = "Doe" } };
        var query = new GetAddressByIdQuery(addressId);

        _mockAddressRepository.Setup(r => r.GetByIdAsync(addressId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Ok(address));
        _mockUserRepository.Setup(r => r.GetByIdAsync(userId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Ok(user));
        _mockAddressMapper.Setup(m => m.Map(address)).Returns(addressDto);
        _mockUserMapper.Setup(m => m.Map(user)).Returns(addressDto.User);

        // Act
        var result = await _handler.Handle(query, CancellationToken.None);

        // Assert
        result.IsSuccess.Should().BeTrue();
        result.Value.Should().Be(addressDto);
    }

    [Fact]
    public async Task Handle_ShouldReturnFail_WhenAddressDoesNotExist()
    {
        // Arrange
        var addressId = 1;
        var query = new GetAddressByIdQuery(addressId);

        _mockAddressRepository.Setup(r => r.GetByIdAsync(addressId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Fail("Address not found"));

        // Act
        var result = await _handler.Handle(query, CancellationToken.None);

        // Assert
        result.IsFailed.Should().BeTrue();
    }

    [Fact]
    public async Task Handle_ShouldReturnFail_WhenUserDoesNotExist()
    {
        // Arrange
        var addressId = 1;
        var userId = 1;
        var address = new Address { Id = addressId, Street = "123 Main St", City = "Anytown", State = "Anystate", PostalCode = "12345", Country = "USA", IsPrimary = true, UserId = userId };
        var query = new GetAddressByIdQuery(addressId);

        _mockAddressRepository.Setup(r => r.GetByIdAsync(addressId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Ok(address));
        _mockUserRepository.Setup(r => r.GetByIdAsync(userId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(Result.Fail("User not found"));

        // Act
        var result = await _handler.Handle(query, CancellationToken.None);

        // Assert
        result.IsFailed.Should().BeTrue();
    }
}





